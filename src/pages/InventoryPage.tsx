...
import { PageHeader, EditableTable, EmptyState, SearchBar, ExportButton } from '@/components/shared';
...
export default function InventoryPage() {
  ...
  const columns: ColumnDef<InventoryItem>[] = [
    ...
    { key: 'value', header: 'القيمة', minWidth: '90px', editable: false, align: 'center',
  ...
  return (
    <div className="space-y-3 sm:space-y-4 lg:space-y-6">
      <PageHeader title="المخزون" subtitle={`${inventory.length} منتج • القيمة: $${formatNumber(totalValue)}`}>
        ...
        <Button onClick={() => addInventoryItem(emptyItem)} className="gradient-primary text-primary-foreground gap-2">
      ...
      {filteredInventory.length === 0 ? (
        ...
        <EditableTable
          ...
          columns={columns}
          onCellChange={updateInventoryField}
          onDeleteRow={deleteInventoryItem}
        />
      )}
    </div>
  );
